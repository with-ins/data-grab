...
export class OnlyBucheonNoticeStep extends AbstractStep {
...
export class OnlyBucheonRecruitStep extends AbstractStep {
    ...
    async execute(page: Page, baseUrl: string): Promise<Record<string, any[]>> {
        ...
        const isEmpty = await page.locator('.cell.empty').count() <= 0;
        if (isEmpty) {
            return null;
        }
