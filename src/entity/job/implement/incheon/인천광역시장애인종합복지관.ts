...
export class 인천광역시장애인종합복지관 extends AbstractJob {
    constructor() {
        super('인천광역시장애인종합복지관', 'https://icjb.or.kr', [
            new 인천공지사항(),
            new 인천이벤트(),
            new 인천채용(),
        ]);
...
class 인천채용 extends SimpleTemplateStep {
    constructor() {
        super('https://icjb.or.kr/bbs/board.php?bo_table=bbs_0403', '.list-item', Category.RECRUIT);
    }
...
class 인천이벤트 extends SimpleTemplateStep {
    ...
    async select(card: Locator, baseUrl: string): Promise<object> {
        ...
        const dateBeforeConvert = (await card.locator('.list-details .hidden-xs').textContent())
            .replace('|', '')
            .trim();
